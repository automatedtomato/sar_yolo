...
class YOLOv3Evaluator:
    ...
    def calculate_iou(self, boxes1: torch.Tensor, boxes2: torch.Tensor, epsilon: float = 1e-10) -> torch.Tensor:
        """
        IoU calculation
    ...
    def postprocess_raw_outputs(
        ...
        for batch_idx in range(batch_size):
            ...
            for scale_idx, output in enumerate(raw_outputs):
                ...
                boxes, scores = self._decode_single_scale(
                    scale_pred, scale_idx
                )
    ...
    def _decode_single_scale(
        ...
        for anchor_idx in range(n_anchors):
            ...
            pred_w = torch.exp(anchor_pred[..., 2]) * self.anchors[scale_idx][anchor_idx][0]
            pred_h = torch.exp(anchor_pred[..., 3]) * self.anchors[scale_idx][anchor_idx][1]
