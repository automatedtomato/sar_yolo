...
def calc_iou(
    ...
    intersection = (inter_rect_x2 - inter_rect_x1).clamp(0) * (
...
def batch_iou(
    self, pred_box: torch.tensor, target_box: torch.tensor, format: str = "xywh"
) -> torch.tensor:
    """
...
def _yolo_loss_single(
    ...
    for i in range(num_anchors):
        ...
        obj_loss = torch.sum(
            -1 * torch.log(torch.sigmoid(obj_pred) + epsilon) * obj_mask
        )

        noobj_mask = ~obj_mask

        noobj_loss = torch.sum(
            -torch.log(1 - torch.sigmoid(obj_pred) + epsilon) * noobj_mask
        )

        class_mask = obj_mask.expand_as(pred_anchor[..., 5:])
        class_loss = F.binary_cross_entropy_with_logits(
            pred_anchor[..., 5:], target_anchor[..., 5:], reduction="none"
        )
