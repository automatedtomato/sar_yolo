...
class YOLOVisualizer:
    ...
    def _annot_to_bbox(self, x_center: float, y_center: float, width: float, height: float, img_width: int, img_height: int) -> list[tuple[int, float, float, float, float]]:
        """
    ...
    def visualize_bbox(self, image_paths: str, annot_paths: str, show_labels: bool=True, fig_size: tuple[int, int]=(12, 8)):
        """
        Visualize YOLO annotations
        ...
        for class_id, x_center, y_center, width, height, label in annotations:
            ...
            x1, y1, x2, y2 = self._annot_to_bbox(x_center, y_center, width, height, image.width, image.height)
            ...
            if show_labels:
                ...
                plt.text(x1, y1-5, label_text, fontsize=6, color='white', bbox=dict(facecolor=color, alpha=0.7, lw=0))
                
            plt.title(f'{os.path.basename(image_paths)}', fontsize=10)
            plt.axis('off')
            plt.show()
    
    def batch_vis_bbox(self, img_ext: str, annot_ext: str, img_prefix: str, annot_prefix: str, batch_size: int, show_labels: bool=True, save_fig: bool=False, save_path: str=None):
        """
        ...
        image_paths, annot_paths = self.data_stream.generate_data(img_ext, annot_ext, img_prefix, annot_prefix, output_size=batch_size, return_sep=True)
