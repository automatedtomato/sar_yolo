...
class SaRDataset(Dataset):
    ...
    def __init__(
        ...
        if len(self.image_paths) != len(self.annot_paths):
            logger.error(
                f"Number of images and annotations must be the same, but got {len(self.image_paths)} images and {len(self.annot_paths)} annotations"
            )
    ...
    def _load_and_parse_annot(self, idx: int):
        try:
            annot_lines = self.data_stream.load_annot(
                self.annot_paths[idx], self.config["data"]["annot_ext"]
            )
    ...
    def _convert_to_target(
        self, annots: list[tuple[int, float, float, float, float, int]]
    ) -> list[torch.Tensor]:
        """
